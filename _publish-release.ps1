...
[xml]$parsedCsproj = Get-Content .\LancacheFileDeleter\LancacheFileDeleter.csproj
...
$currentContent = Get-Content -Path .\LancacheFileDeleter\LancacheFileDeleter.csproj -Raw
$currentContent = $currentContent.Replace('<VersionPrefix>' + $currentVersion, '<VersionPrefix>' + $newVersion)
Set-Content -Value $currentContent -Path .\LancacheFileDeleter\LancacheFileDeleter.csproj -NoNewline
