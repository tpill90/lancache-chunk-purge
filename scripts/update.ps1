...
# Seeing if LancacheFileDeleter is already installed and up to date
if(Test-Path "LancacheFileDeleter.exe")
{
    $currentVersion = (.\LancacheFileDeleter.exe --version)
...
Copy-Item "$($windowsAsset.name.Replace('.zip', ''))\LancacheFileDeleter.exe"
