...
# Checking to see if LancacheFileDeleter is already up to date
if [ -f ./LancacheFileDeleter ]; then
    CURRENT_VERSION=$(./LancacheFileDeleter --version)
...
DOWNLOAD_URL="https://github.com/tpill90/steam-lancache-prefill/releases/download/v${LATEST_TAG}/LancacheFileDeleter-${LATEST_TAG}-linux-x64.zip"
wget -q -nc --show-progress --progress=bar:force:noscroll $DOWNLOAD_URL

# Unzip
echo -e "${Yellow} Unzipping... ${NC}"
unzip -q -j -o LancacheFileDeleter-${LATEST_TAG}-linux-x64.zip

# Required so executable permissions don't get overwritten by unzip
chmod +x LancacheFileDeleter update.sh

# Cleanup
rm LancacheFileDeleter-${LATEST_TAG}-linux-x64.zip
